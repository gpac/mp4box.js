 ...
 */
import type { ParseOneBoxResponse } from './types';

import { BoxParser, _BoxParser, Box, FullBox, ContainerBox } from './box';
import { BoxStream } from './buffer';
import { Log } from './log';

function parseUUID(stream: BoxStream) {
  return parseHex16(stream);
}

function parseHex16(stream: BoxStream) {
  let hex;
  let hex16 = '';
  for (let i = 0; i < 16; i++) {
    hex = stream.readUint8().toString(16);
...
export default {
  ...
  Box: {
    parse: function (this: Box, stream: BoxStream) {
      if (this.type != 'mdat') {
    ...
    parseDataAndRewind: function (this: Box, stream: BoxStream) {
      this.data = stream.readUint8Array(this.size - this.hdr_size);
      // rewinding
      stream.position -= this.size - this.hdr_size;
    },
    parseLanguage: function (this: Box, stream: BoxStream) {
  ...
  FullBox: {
    parseDataAndRewind: function (this: FullBox, stream: BoxStream) {
    ...
    parseFullHeader: function (this: FullBox, stream: BoxStream) {
      this.version = stream.readUint8();
      this.flags = stream.readUint24();
      this.hdr_size += 4;
    },
    parse: function (this: FullBox, stream: BoxStream) {
      this.parseFullHeader(stream);
      this.data = stream.readUint8Array(this.size - this.hdr_size);
    },
  },
  ContainerBox: {
    parse: function (this: ContainerBox, stream: BoxStream) {
      var ret;
      var box;
      while (stream.getPosition() < this.start + this.size) {
        ...
        if (ret.code === BoxParser.OK) {
          ...
          if (this.subBoxNames && this.subBoxNames.indexOf(box.type) != -1) {
            const prop = (this.subBoxNames[this.subBoxNames.indexOf(box.type)] +
              's') as keyof ContainerBox;
            this[prop].push(box);
          } else {
            const box_type = (box.type !== 'uuid' ? box.type : box.uuid) as keyof ContainerBox;
            if (this[box_type]) {
              Log.warn('Box of type ' + box_type + ' already stored in field of this type');
            } else {
              this[box_type] = box as never;
            }
          }
        } else {
          return;
