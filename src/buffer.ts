...
import type { BoxRange } from './types';
...
export type BoxStream = MultiBufferStream | MP4BoxStream;
...
export class MultiBufferStream extends DataStream {
  ...
  constructor(buffer?: BoxBuffer) {
    super(new BoxBuffer(0), 0, DataStream.BIG_ENDIAN);
    /* List of ArrayBuffers, with a fileStart property, sorted in fileStart order and non overlapping */
    if (buffer) {
      this.insertBuffer(buffer);
      this.bufferIndex = 0;
    }
    const _buffer = new ArrayBuffer(0) as BoxBuffer;
    _buffer.fileStart = 0;
    _buffer.usedBytes = 0;
    this.buffer = _buffer;
  }
  ...
  ***********************************************************************************/
   ...
   */
  static concat(buffer1: BoxBuffer, buffer2: BoxBuffer): BoxBuffer {
    Log.debug(
      'ArrayBuffer',
    ...
    const buffer = tmp.buffer as BoxBuffer;
    return buffer;
  }
   ...
   */
  reduceBuffer(buffer: BoxBuffer, offset: number, newLength: number) {
    const smallB = new Uint8Array(newLength);
    smallB.set(new Uint8Array(buffer, offset, newLength));
    const smallBuffer = smallB.buffer as BoxBuffer;
    smallBuffer.fileStart = buffer.fileStart + offset;
    smallBuffer.usedBytes = 0;
    return smallBuffer;
  }
   ...
   */
  logBufferLevel(info?: any) {
    let i;
    let buffer;
    let used = 0;
    let total = 0;
    let range: BoxRange = { start: 0, end: 0 };
    let bufferedString = '';
    ...
    const log = info
      ? (module: string, msg: string) => Log.info(module, msg)
      : (module: string, msg: string) => Log.debug(module, msg);
  ...
  mergeNextBuffer() {
    ...
    if (this.bufferIndex + 1 < this.buffers.length) {
      ...
      if (next_buffer.fileStart === this.buffer.fileStart + this.buffer.byteLength) {
        ...
        this.buffers[this.bufferIndex] = MultiBufferStream.concat(this.buffer, next_buffer);
 ...
 *************************************************************************/
  ...
  seek(filePosition: number, fromStart?: boolean, markAsUsed?: boolean) {
    const index = this.findPosition(Boolean(fromStart), filePosition, Boolean(markAsUsed));
    if (index !== -1) {
