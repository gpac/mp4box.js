...
import { BoxParser, Box, _BoxParser, SampleEntry } from './box';
import { DataStream } from './DataStream';
import { MP4BoxStream } from './stream';
import { Sample } from './types';

export default {
  init: function (this: ISOFile, _options: any) {
  ...
  addTrack: function (this: ISOFile, _options: any) {
    if (!this.moov) {
      this.init(_options);
    }
    ...
    const prop = (options.type + 'SampleEntry') as keyof _BoxParser;
    if (BoxParser[prop] === undefined) return;
    var sample_description_entry = new BoxParser[prop]() as SampleEntry;
    sample_description_entry.data_reference_index = 1;
    var media_type = '';
    for (var mediaType of Object.keys(BoxParser.sampleEntryCodes)) {
    ...
    switch (media_type) {
      case 'Visual':
        ...
        if (options.avcDecoderConfigRecord) {
          var avcC: Box = new BoxParser.avcCBox();
    ...
    if (options.description_boxes) {
      options.description_boxes.forEach(function (b: Box) {
  ...
  addSample: function (this: ISOFile, track_id: number, data: ArrayBuffer, _options?: any) {
    var options = _options || {};
    var sample: any = {};
    ...
    var moof: Box = this.createSingleSampleMoof(sample);
    this.addbox(moof);
    moof.computeSize();
  ...
  createSingleSampleMoof: function (this: ISOFile, sample: Sample) {
    var sample_flags = 0;
  ...
  computeSize: function (this: Box, stream_?: DataStream) {
