...
In order to use the `MP4Box.js` in a browser, you can either include all files from the ```src``` folder or use grunt to build a single-file library, minified (```mp4box.all.min.js```) or not (```mp4box.all.js```), stored in the ```dist``` folder.
...
NOTE: a smaller size version of the library for box parsing only is also generated by grunt: minified (```mp4box.simple.min.js```) or not (```mp4box.simple.js```). This is work in progress.
