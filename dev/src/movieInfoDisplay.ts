import { Log, type Movie, type Track } from 'mp4box';
...
function getBasicTrackInfo(track: Track) {
  var i;
  var html = '';
  html += '<td>' + track.id + '</td>';
  html += '<td>';
  if (track.references && track.references.length > 0) {
    ...
    for (i = 0; i < track.references.length; i++) {
      html +=
        '<tr><td>' +
        track.references[i].type +
        '</td><td>' +
        track.references[i].track_ids +
        '</td></tr>';
    }
    html += '</tbody>';
    html += '</table>';
  ...
  html +=
    ...
    ' - ' +
    Log.getDurationString(track.movie_duration!, track.movie_timescale) +
    '</td>';
  html += '<td>';
  if (track.edits && track.edits.length > 0) {
    ...
    for (i = 0; i < track.edits.length; i++) {
      ...
      html +=
        '<td>' +
        track.edits[i].segment_duration +
        ' - ' +
        Log.getDurationString(track.edits[i].segment_duration, track.movie_timescale) +
        '</td>';
      if (track.edits[i].media_time !== -1) {
        html +=
          '<td>' +
          track.edits[i].media_time +
          ' - ' +
          Log.getDurationString(track.edits[i].media_time, track.timescale) +
          '</td>';
      } else {
        html += '<td></td>';
      }
  ...
  html +=
    ...
    track.duration +
    ' - ' +
    Log.getDurationString(track.duration!, track.timescale) +
    '</td>';
  ...
  if (track.cts_shift) {
    ...
    html +=
      '<tr><td>shift</td><td>' +
      track.cts_shift.compositionToDTSShift +
      ' - ' +
      Log.getDurationString(track.cts_shift.compositionToDTSShift, track.timescale!) +
      '</td></tr>';
    html +=
      '<tr><td>min</td><td>' +
      track.cts_shift.leastDecodeToDisplayDelta +
      ' - ' +
      Log.getDurationString(track.cts_shift.leastDecodeToDisplayDelta, track.timescale!) +
      '</td></tr>';
    html +=
      '<tr><td>max</td><td>' +
      track.cts_shift.greatestDecodeToDisplayDelta +
      ' - ' +
      Log.getDurationString(track.cts_shift.greatestDecodeToDisplayDelta, track.timescale) +
      '</td></tr>';
    html +=
      '<tr><td>start</td><td>' +
      track.cts_shift.compositionStartTime +
      ' - ' +
      Log.getDurationString(track.cts_shift.compositionStartTime, track.timescale) +
      '</td></tr>';
    html +=
      '<tr><td>end</td><td>' +
      track.cts_shift.compositionEndTime +
      ' - ' +
      Log.getDurationString(track.cts_shift.compositionEndTime, track.timescale) +
      '</td></tr>';
    html += '</table>';
  }
  html += '</td>';
...
function getVideoTrackInfo(track: Track) {
  var html = '';
...
function getAudioTrackInfo(track: Track) {
  var html = '';
...
function generateTrackHeader(type: any) {
  var html = '';
  html += '<tr>';
...
function generateTrackInfo(
  track: Track,
  type: 'Video' | 'Audio' | 'Subtitle' | 'Metadata' | 'Hint',
) {
  var html = '';
  html += '<tr>';
  ...
  if (displaySourceBuffer) {
    ...
    if (MediaSource.isTypeSupported(mime)) {
      ...
      html +=
        '<td>Not supported by your browser, exposing track content using HTML TextTrack <input id="addTrack' +
        track.id +
        '" type="checkbox"></td>';
    }
  }
...
export function displayMovieInfo(
  info: Movie,
  div: HTMLElement,
  _displaySourceBuffer: boolean | undefined,
) {
  ...
  html += '<table>';
  html +=
    '<tr><th>File Size / Bitrate</th><td>' +
    fileLength +
    ' bytes / ' +
    Math.floor((fileLength * 8 * info.timescale!) / (info.duration! * 1000)) +
    ' kbps</td></tr>';
  if (info.timescale) {
    html +=
      '<tr><th>Duration / Timescale</th><td>' +
      info.duration +
      '/' +
      info.timescale +
      ' (' +
      Log.getDurationString(info.duration!, info.timescale) +
      ')</td></tr>';
  }
  ...
  if (info.created && info.modified) {
    html +=
      '<tr><th>Creation / Modification Dates</th><td>' +
      dateToInput(info.created) +
      ' / ' +
      dateToInput(info.modified) +
      '</td></tr>';
  }
  html += '</table>';
  if (info.videoTracks) {
...
function dateToInput(date: Date) {
  ...
  var y: string | number = date.getFullYear();
  var h: string | number = date.getHours();
  var mn: string | number = date.getMinutes();
  if (d < 10) {
    d = '0' + d;
  }
  if (m < 10) {
