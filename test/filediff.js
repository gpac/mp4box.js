...
function getFancyTreeDiffedDataFromBoxes(boxes, expected_boxes) {
  ...
  for (var i = 0; i < boxes.length; i++) {
    ...
    if (expected_boxes) {
      for (j = 0; j < expected_boxes.length; j++) {
        if (
          expected_boxes[j].type === box.type &&
          expected_boxes[j].uuid === box.uuid &&
          !expected_boxes_compared[j]
        ) {
    ...
    for (j = 0; j < child_prop_names.length; j++) {
      ...
      if (box[name]) {
        ...
        if (name === 'esd') {
          fancytree_node.children = getFancyTreeDataFromDesc(
            box[name],
            expected_box ? expected_box[name] : expected_box,
          );
        } else {
          fancytree_node.children = getFancyTreeDiffedDataFromBoxes(
            box[name],
            expected_box ? expected_box[name] : expected_box,
          );
        }
...
function finalizeDiffUI(fileobj, loadbutton, success) {
  ...
  if (success) {
    if (file_a.mp4boxfile != null && file_b.mp4boxfile != null) {
      var treeboxes = getFancyTreeDiffedDataFromBoxes(
        file_a.mp4boxfile.boxes,
        file_b.mp4boxfile.boxes,
      );
