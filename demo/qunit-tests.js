...
QUnit.test('appending invalid buffer', function (assert) {
  ...
  assert.throws(function () {
    mp4boxfile.appendBuffer(null);
  }, 'Exception thrown because of null buffer');
  assert.throws(function () {
    mp4boxfile.appendBuffer(new ArrayBuffer());
  }, 'Exception thrown because of missing fileStart property');
...
QUnit.asyncTest(
  'appending 2 non-overlapping chunks (mid-moov cut, in order: 1 2)',
  function (assert) {
    var index = 0;
    var timeout = window.setTimeout(function () {
      assert.ok(false, 'Timeout');
...
QUnit.asyncTest(
  'appending 2 non-overlapping chunks (mid-mdat cut, in order: 1 2)',
  function (assert) {
    var index = 0;
    var timeout = window.setTimeout(function () {
      assert.ok(false, 'Timeout');
...
QUnit.asyncTest(
  'appending 2 non-overlapping chunks (mid-moov cut, out-of-order: 2 1)',
  function (assert) {
    var index = 0;
    var timeout = window.setTimeout(function () {
      assert.ok(false, 'Timeout');
...
QUnit.asyncTest(
  'appending 2 non-overlapping chunks (mid-mdat cut, out-of-order: 2 1)',
  function (assert) {
    var index = 0;
    var timeout = window.setTimeout(function () {
      assert.ok(false, 'Timeout');
...
QUnit.asyncTest(
  'appending 3 non-overlapping chunks (mid-moov cut, order: 1 3 2)',
  function (assert) {
    var index = 0;
    var timeout = window.setTimeout(function () {
      assert.ok(false, 'Timeout');
...
QUnit.asyncTest(
  'appending 3 non-overlapping chunks (mid-mdat cut, order: 1 3 2)',
  function (assert) {
    var index = 0;
    var timeout = window.setTimeout(function () {
      assert.ok(false, 'Timeout');
...
QUnit.asyncTest(
  'appending 2 overlapping chunks (mid-mdat cut, incomplete mdat, in order: 1 2)',
  function (assert) {
    var index = 0;
    var timeout = window.setTimeout(function () {
      assert.ok(false, 'Timeout');
...
QUnit.asyncTest(
  'appending 2 overlapping chunks (mid-mdat cut, out of order: 2 1)',
  function (assert) {
    var index = 0;
    var timeout = window.setTimeout(function () {
      assert.ok(false, 'Timeout');
...
QUnit.asyncTest(
  'appending only one buffer with fileStart different from zero should not reach onReady',
  function (assert) {
    var index = 0;
    var mp4boxfile = MP4Box.createFile();
    QUnit.expect(0);
...
QUnit.asyncTest(
  'Segmentation when no sample is ready should not reach onSegment',
  function (assert) {
    var index = 0;
    var track_id;
    var mp4boxfile = MP4Box.createFile();
    var timeout = window.setTimeout(function () {
      assert.ok(true, 'No onSegment reached after timeout expiration');
      QUnit.start();
    }, TIMEOUT_MS);
    mp4boxfile.onSegment = function (id, user, buffer, sampleNum) {
    ...
    getFileRange(testFiles[index].url, 20, 39, function (buffer) {
      ...
      getFileRange(testFiles[index].url, 40, 100, function (buffer) {
        ...
        assert.equal(
          next_pos,
          1309934 + 40,
          'Next position after third append corresponds to moov position',
        );
        QUnit.start();
      });
    });
...
QUnit.asyncTest('mdat progressive download', function (assert) {
  ...
  getFileRange(testFiles[index].url, 0, 79999, function (buffer) {
    ...
    assert.equal(
      next_pos,
      80000,
      'Next position after first append corresponds to end of previous buffer (moov entirely parsed)',
    );
    getFileRange(testFiles[index].url, 80000, 119999, function (buffer) {
      ...
      assert.equal(
        next_pos,
        120000,
        'Next position after second append corresponds to end of previous buffer (contiguous append)',
      );
      getFileRange(testFiles[index].url, 200000, 259999, function (buffer) {
        ...
        assert.equal(
          next_pos,
          120000,
          'Next position after third append corresponds to end of second buffer (non-contiguous append)',
        );
        getFileRange(testFiles[index].url, 120000, 199999, function (buffer) {
          ...
          assert.equal(
            next_pos,
            260000,
            'Next position after fourth append corresponds to end of all buffer (all-contiguous)',
          );
          QUnit.start();
        });
      });
...
QUnit.asyncTest('Seek without moov', function (assert) {
  ...
  getFileRange(testFiles[index].url, 0, 10, function (buffer) {
    ...
    }, 'Exception thrown because moov not found');
    window.clearTimeout(timeout);
    QUnit.start();
  });
});
...
function makeBoxParsingTest(i) {
  ...
  QUnit.asyncTest(boxtests[boxtestIndex].boxname, function (assert) {
    ...
    var rangeEnd =
      boxtests[boxtestIndex].rangeSize + boxtests[boxtestIndex].rangeStart - 1 || Infinity;
...
QUnit.asyncTest('moov-first, parsed, append a non-contiguous buffer', function (assert) {
  ...
  getFileRange(testFiles[index].url, 0, testFiles[index].moovEnd, function (buffer) {
    ...
    getFileRange(
      testFiles[index].url,
      testFiles[index].moovEnd + 100,
      testFiles[index].moovEnd + 1000,
      function (buffer) {
        window.clearTimeout(timeout);
        var nextFileStart = mp4boxfile.appendBuffer(buffer);
        assert.equal(
          nextFileStart,
          testFiles[index].moovEnd + 1,
          'next parsing position should be immediately after the moov',
        );
        QUnit.start();
      },
    );
...
QUnit.asyncTest('Moov-last, parsed, append buffer not starting at mdat start', function (assert) {
  ...
  getFileRange(testFiles[index].url, 0, testFiles[index].mdatStart + 8 - 1, function (buffer) {
    ...
    assert.equal(
      nextStart,
      testFiles[index].mdatStart + testFiles[index].mdatSize,
      'next parsing position should be after mdat',
    );
    // fetching the moov box
    getFileRange(
      testFiles[index].url,
      testFiles[index].mdatStart + testFiles[index].mdatSize,
      nextStart + testFiles[index].moovSize - 1,
      function (buffer) {
        window.clearTimeout(timeout);
        nextStart = mp4boxfile.appendBuffer(buffer);
        assert.equal(
          nextStart,
          testFiles[index].mdatStart + testFiles[index].mdatSize + testFiles[index].moovSize,
          'Next parse position should be after moov',
        );
        QUnit.start();
      },
    );
