...
function setMP4Box() {
  ...
  mp4boxfile.onReady = function (info) {
    ...
    mp4boxfile.start();
    if (!isHEVC) throw 'Not an HEVC movie file.';
  };
      ...
      for (var i = 0; i < samples.length; i++) {
        ...
        if (sample.description.type === 'hvc1') {
          if (sample.is_sync === true) {
            ...
            bpg.readFromHEVC(
              hevcFrame,
              sample.size + sample.description.hvcC.size,
...
function loadFromFile() {
  ...
  // HEVC(MP4)
  if (file.type === 'video/mp4') loadVideoFileUpload(file);
  // BPG
  else if (file.name.split('.').pop() === 'bpg') loadImageFileUpload(file);
  else throw 'Not a valid file.';
...
function loadFromHttpUrl() {
  ...
  if (url.length > 3) {
    ...
    // HEVC(MP4)
    if (extension === 'mp4') loadVideoFileHttpUrl(url);
    // BPG
    else if (extension === 'bpg') loadImageFileHttpUrl(url);
    else validUrl = false;
  } else validUrl = false;

  if (!validUrl) throw 'Not a valid HTTP URL.';
}
...
window.onload = function () {
  ...
  $(window).scroll(function () {
    if ($(this).scrollTop() > 283) progressBar.addClass('fixed-menu');
    else progressBar.removeClass('fixed-menu');
  });
  ...
  if (URL && URL.createObjectURL) {
    ...
    img.onload = function () {
      ...
      if (isThumbnail) {
        ...
        for (
          var imageBufferedDTS = imagesBuffer.shift();
...
function buildThumbnail(bpg, imageData, dts, canvas) {
  ...
  if (canvas) {
    ...
    canvasTimeline.addEventListener(
      'click',
      function () {
        buildImageDiv(bpg, canvas);
      },
      false,
    );
    canvasTimeline.addEventListener(
    ...
    canvasTimeline.addEventListener(
      'mouseout',
      function () {
        $(this).removeClass('selected-thumbnail');
      },
      false,
    );
  } else {
    ...
    ctxTimeline.fillText(
      'Could not decode image',
      canvasTimeline.width / 2,
      canvasTimeline.height / 2,
    );
  }
...
function buildImageDiv(bpg, imageData, canvas) {
  ...
  downloadButton.addEventListener(
    'click',
    function () {
      saveData(bpg.toBitStream().dataView.buffer, 'image.bpg');
    },
    false,
  );
