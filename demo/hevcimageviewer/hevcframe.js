...
HEVCStream.prototype.writeSPS = function () {
  ...
  nbBits += bs.numToExpGolomb(
    this.SPS.log2_min_luma_coding_block_size_minus3,
  ); /* log2_min_luma_coding_block_size_minus3 */
  nbBits += bs.numToExpGolomb(
    this.SPS.log2_diff_max_min_luma_coding_block_size,
  ); /* log2_diff_max_min_luma_coding_block_size */
  nbBits += bs.numToExpGolomb(
    this.SPS.log2_min_transform_block_size_minus2,
  ); /* log2_min_luma_transform_block_size_minus2 */
  nbBits += bs.numToExpGolomb(
    this.SPS.log2_diff_max_min_transform_block_size,
  ); /* log2_diff_max_min_luma_transform_block_size */
  nbBits += bs.numToExpGolomb(
    this.SPS.max_transform_hierarchy_depth_inter,
  ); /* max_transform_hierarchy_depth_inter */
  nbBits += bs.numToExpGolomb(
    this.SPS.max_transform_hierarchy_depth_intra,
  ); /* max_transform_hierarchy_depth_intra */
  bs.dataView.writeUnsigned(0, 1); /*scaling_list_enabled_flag */
  bs.dataView.writeUnsigned(1, 1); /*amp_enabled_flag */
  bs.dataView.writeUnsigned(
    this.SPS.sample_adaptive_offset_enabled_flag,
    1,
  ...
  if (this.SPS.pcm_enabled_flag) {
    bs.dataView.writeUnsigned(
      this.SPS.pcm_sample_bit_depth_luma_minus1,
      4,
    ); /*pcm_sample_bit_depth_luma_minus1*/
    nbBits += 4;
    bs.dataView.writeUnsigned(
      this.SPS.pcm_sample_bit_depth_chroma_minus1,
      4,
    ); /*pcm_sample_bit_depth_chroma_minus1*/
    nbBits += 4;
    nbBits += bs.numToExpGolomb(
      this.SPS.log2_min_pcm_luma_coding_block_size_minus3,
    ); /* log2_min_pcm_luma_coding_block_size_minus3 */
    nbBits += bs.numToExpGolomb(
      this.SPS.log2_diff_max_min_pcm_luma_coding_block_size,
    ); /* log2_diff_max_min_pcm_luma_coding_block_size */
    bs.dataView.writeUnsigned(
      this.SPS.pcm_loop_filter_disabled_flag,
      4,
  ...
  bs.dataView.writeUnsigned(
    this.SPS.strong_intra_smoothing_enabled_flag,
    1,
  ...
  if (this.SPS.sps_range_extension_flag) {
    bs.dataView.writeUnsigned(
      this.SPS.transform_skip_rotation_enabled_flag,
      1,
...
HEVCStream.prototype.readSPS = function (nalu) {
  ...
  if (this.SPS.sps_extension_present_flag) {
    ...
    this.SPS.sps_extension_7bits =
      bitStreamRead.dataView.getUnsigned(1) * 128 + bitStreamRead.dataView.getUnsigned(6);
  }
  if (this.SPS.sps_range_extension_flag) {
...
HEVCStream.createStreamFromSample = function (sample) {
  ...
  hevcstream.data = NALUStream.readSampleData(
    mp4NALUSData,
    sample.description.hvcC.lengthSizeMinusOne + 1,
  );
  return hevcstream;
};
